...
float3 applyNoir(float3 color, float intensity) {
    float luma = dot(color, float3(0.299, 0.587, 0.114));
    float3 noir = float3(luma, luma, luma);
    float3 tinted = float3(luma * 1.1, luma * 1.0, luma * 0.9);
    return mix(color, mix(noir, tinted, 0.3), clamp(intensity, 0.0, 1.0));
}

float3 applyChrome(float3 color, float intensity) {
    float luma = dot(color, float3(0.299, 0.587, 0.114));
    float3 chrome = float3(luma * 1.2, luma * 1.1, luma * 0.9);
    chrome = clamp(chrome, 0.0, 1.0);
    return mix(color, chrome, clamp(intensity, 0.0, 1.0));
}
...
float3 applyInstant(float3 color, float intensity) {
    ...
    return mix(color, clamp(faded, 0.0, 1.0), clamp(intensity, 0.0, 1.0));
}
    ...
    for (uint i = 0; i < count; i++) {
        ...
        switch (f.type) {
            ...
            case 2:
                ...
                break;
            case 3:
                color = applyNoir(color, f.intensity);
                break;
            case 4:
                color = applyChrome(color, f.intensity);
                break;
            case 5:
                color = applyInstant(color, f.intensity);
                break;
            default:
                break;
        }
